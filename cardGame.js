...
function compare(currentCard, nextCard) {
  console.log(currentCard.value - nextCard.value)
...
function guess(currentCard, nextCard) {
  ...
  console.log(`The next card is the ${nextCard.rank} of ${nextCard.suit} with value ${nextCard.value}!`); // TEST
  let input = getInput("Will the next card be higher or lower than the current card? Type h for higher or l for lower");
  if (input.toLowerCase() != 'h' && input.toLowerCase() != 'l') {
    console.log("Sorry! You did not enter a valid guess! You need to type h for higher or l for lower. You get no points!");
    return false;
  } else if (input.toLowerCase === 'h') {
    console.log(`Compare equals ${compare(currentCard, nextCard)}!`);
    console.log(`Compare function returned ${((compare(currentCard, nextCard) < 0) ? true : false)}!`);
    return ((compare(currentCard, nextCard) < 0) ? true : false);
  } else {
    console.log(`Compare equals ${compare(currentCard, nextCard)}!`);
    console.log(`Compare function returned ${((compare(currentCard, nextCard) > 0) ? true : false)}!`);
    return ((compare(currentCard, nextCard) > 0) ? true : false);
...
function playGame() {
  ...
  while (score < 5 || score < deck.length) {
    ...
    if (guess(currentCard, nextCard)) {
      ...
      console.log(`Too bad! Your guess was incorrect and you got no points!`);
    }
